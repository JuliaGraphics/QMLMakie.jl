...
mutable struct QMLScreen <: GLMakie.GLScreen
  ...
  shader_cache::GLMakie.GLAbstraction.ShaderCache
...
function GLMakie.render_frame(screen::QMLScreen; resize_buffers=true)
  ...
  glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE)
  glStencilMask(0x00)
  GLMakie.GLAbstraction.render(screen) do robj
...
function to_qmlscreen_postprocessor(framebuffer, shader_cache)
  # draw color buffer
  shader = GLMakie.GLAbstraction.LazyShader(
    shader_cache,
  ...
  full_render = screen -> begin
    ...
    glViewport(0, 0, w, h)
    glClear(GL_COLOR_BUFFER_BIT)
    GLMakie.GLAbstraction.render(pass) # copy postprocess
  end
